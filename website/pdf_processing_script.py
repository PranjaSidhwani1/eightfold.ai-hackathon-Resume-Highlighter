...
def highlight_karde_bhai(input_resume_path,output_pdf_path,input_query):
    your_token = ""
    print(input_query)
    ...
    for j in range (0,len(line_text)):
        print(line_text[j])
    return highlight_pdf(input_pdf_path, output_pdf_path, line_text)
...
if __name__ == '__main__':
    if len(sys.argv) != 4:
        print('Usage: python pdf_processing_script.py input_file output_file')
        sys.exit(1)
    #print("abhi tk maa nahi chudi hai")
    input_file = sys.argv[1]
    output_file = sys.argv[2]
    query=sys.argv[3]
    highlight_karde_bhai(input_file,output_file,query)
